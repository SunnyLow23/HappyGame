...
int SDLCommonFunc::ShowMenu(SDL_Renderer *screen_sdl, TTF_Font *font,
                            ...
                            Mix_Chunk *effect_sound[3], Mix_Music *background_music)
{
    ...
    text_object[0].LoadFromRenderText(font, screen_sdl);

    text_object[1].SetText(labels[1]);
    text_object[1].SetColor(color[0].r, color[0].g, color[0].b);
    text_object[1].LoadFromRenderText(font, screen_sdl);

    SDL_Rect pos[kMenuNum];
    pos[0].x = SCREEN_WIDTH*0.4 - 150;
    pos[0].y = SCREEN_HEIGHT*0.5 - 40;
    pos[1].x = SCREEN_WIDTH*0.4 - 150;
    pos[1].y = SCREEN_HEIGHT*0.5 + 40;

    BaseObject gBackground;
    bool ret = gBackground.LoadImg(img_file, screen_sdl);
    ...
    while (1)
    {
                ...
                for (int i = 0; i < kMenuNum; ++i)
                {
                        ...
                        y >= pos[i].y && y <= pos[i].y + pos[i].h)
                    {
                        if (!selected[i])
                        {
                            ...
                            Mix_PlayChannel(-1, effect_sound[0], 0);
                            text_object[i].SetColor(color[1].r, color[1].g, color[1].b);
                            text_object[i].LoadFromRenderText(font, screen_sdl);
                        }
                    }
                    else
                    {
        ...
        gBackground.Render(screen_sdl, NULL);

        for (int i = 0; i < kMenuNum; ++i)
        {
            text_object[i].RenderText(screen_sdl, pos[i].x, pos[i].y);
